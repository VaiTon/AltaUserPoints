...
class com_AltaUserPointsInstallerScript
{
        function postflight($type, $parent) 
        {
				...
				$q = "SELECT id FROM #__modules WHERE module='mod_aupadmin' AND published='1'";
				$db->setQuery( $q );
				$idmodule = $db->LoadResult();
				if ($idmodule)
				{	
					$q = "UPDATE #__modules SET published='0' 
				...
				if( $plugin_installer->install($file_origin) ) {
					// publish plugin
					$q = "UPDATE #__extensions SET enabled='1' 
				...
				if($plugin_installer->install($file_origin)) {
					// publish plugin
					$q = "UPDATE #__extensions SET enabled='1', ordering='999' 
				...
				{
					// publish plugin
					$q = "UPDATE #__extensions SET enabled='1' WHERE `element`='notification_rank_medal' 
				...
				if ( $error )
				{
			     ...
			     JControllerLegacy::redirect(); 
					...
					$q = "SELECT id FROM #__alpha_userpoints WHERE `userid`='0' AND `referreid`='GUEST'";
					$db->setQuery( $q );
					$result = $db->loadResult();
					if ( !$result ) {
						// This GUEST user is used by AUP system, don't remove!
						$q = "INSERT INTO #__alpha_userpoints (`id`, `userid`, `referreid`, `points`, 
					...
					if ( !$result ) {		
						...
						$q = "SELECT version FROM #__alpha_userpoints_version WHERE 1";
						$db->setQuery( $q );
						$result = $db->loadResult();
						if ( !$result ) {
							$q = "INSERT INTO #__alpha_userpoints_version (`version`) VALUES ('AUP11');";
							$db->setQuery( $q );
							$db->execute();
						}
					...
					$q = "SELECT COUNT(*) FROM #__alpha_userpoints_levelrank";
					$db->setQuery( $q );
					...
					if ( !$nblevelrank ) {
						...
						$q = "INSERT INTO `#__alpha_userpoints_levelrank` (`id`, `rank`, `description`, 
				...
				foreach ($modules as $module)
				{
					...
					if($client == 'administrator') {
							...
							$sql = $db->getQuery(true)
								...
								// Link to all pages
								$q = $db->getQuery(true);
								$q->select('id')->from($db->qn('#__modules'))
									->where($db->qn('module').' = '.$db->q($name));
								$db->setQuery($q);
								$moduleid = $db->loadResult();
		
								$q = $db->getQuery(true);
								$q->select('*')->from($db->qn('#__modules_menu'))
