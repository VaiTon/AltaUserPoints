			...
			<ul class="treeselect">
				...
				<?php if (count($type->links)) : ?>
					...
					<?php foreach ($type->links as $i => $link) : ?>							
						<?php
						// added
						$menuSelected = explode(',', $this->row->content_items);
						...
						$selected = 0;
						if ($this->row->content_items == 0)
						{
							$selected = 1;
						} elseif ($this->row->content_items < 0)
						{
							$selected = in_array(-$link->value, $menuSelected);
						} elseif ($this->row->content_items > 0)
						{
							$selected = in_array($link->value, $menuSelected);
						}
						?>
							<li>
								<div class="treeselect-item pull-left">
									<input type="checkbox" class="pull-left" name="content_items[]" id="<?php echo $id . $link->value; ?>" value="<?php echo (int) $link->value; ?>"<?php echo $selected ? ' checked="checked"' : ''; ?> />
