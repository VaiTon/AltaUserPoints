 ...
 * @copyright Copyright (C) 2015-2017 Bernard Gilly - Adrien Roussel
 * @license : GNU/GPL
		...
		if ( $isnew )
		{
			...
			{			
				...
				if ( $result[0]->displaymsg ) 
				{
					...
					if ( $msg!='' )
					{								
						$app->enqueueMessage( str_replace ( '{points}', AltaUserPointsHelper::getFPoints($result[0]->points), JText::_( $msg ) ),'message');
					} else {
						$app->enqueueMessage( sprintf ( JText::_('AUP_CONGRATULATIONS_PLURAL'), $result[0]->points ),'message');
					}			
				}
